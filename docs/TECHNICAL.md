...
│   │   ├── auth.ts         # ACCESS_TOKEN 登录/鉴权
│   │   ├── active.ts       # 流式生成 AbortController 注册表
...
- `ACCESS_TOKEN` 是公网部署的第一道防线；生产环境务必设置。
- API Key 存储在后端 SQLite/环境变量，`/api/connections` 返回时已脱敏（`api_key` 为空，`has_api_key` 表示是否存在）。
- 如果通过公网直接暴露 3000 端口，建议用 Nginx/Caddy 加 HTTPS，并参考 `deploy/` 示例关闭 SSE 缓冲。
